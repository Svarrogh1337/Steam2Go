...
func TestApps(t *testing.T) {
	...
	assert.Nil(t, err)
	sdr2, err := c.GetSDRConfigV1(context.Background(), 440)
	assert.NotNil(t, sdr2.Pops.Ams.Relays[0].Ipv4)
	assert.Nil(t, err)
	server, err := c.GetServersAtAddressV1(context.Background(), "194.182.177.228:27310")
	assert.True(t, server.Response.Success)
	assert.Nil(t, err)
	server2, err := c.GetServersAtAddressV1(context.Background(), "194.182.177.228")
